...
#define EHCI_MSM_HSIC_DEBUG 0
#if EHCI_MSM_HSIC_DEBUG
	#define D(x...) dev_info(x)
#else
	#define D(x...) dev_dbg(x)
#endif
...
static bool skip_resume_enabled = false;
...
static int msm_hsic_suspend(struct msm_hsic_hcd *mehci)
{
	...
	if (test_bit(HCD_FLAG_WAKEUP_PENDING, &hcd->flags) ||
		...
		D(mehci->dev, "wakeup pending, aborting suspend\n");
		enable_irq(hcd->irq);
		return -EBUSY;
	}
	...
	D(mehci->dev, "HSIC-USB in low power mode\n");
...
static int msm_hsic_resume(struct msm_hsic_hcd *mehci)
{
	...
	if (!atomic_read(&mehci->in_lpm)) {
		D(mehci->dev, "%s called in !in_lpm\n", __func__);
		return 0;
	}
...
skip_phy_resume:
	...
	enable_irq(hcd->irq);
	D(mehci->dev, "HSIC-USB exited from low power mode\n");
...
static int ehci_hsic_bus_suspend(struct usb_hcd *hcd)
{
	...
	if (!(readl_relaxed(USB_PORTSC) & PORT_PE)) {
		...
		D(mehci->dev, "%s:port is not enabled skip suspend\n",
				__func__);
		return -EAGAIN;
	}
...
static int ehci_hsic_bus_resume(struct usb_hcd *hcd)
{
	...
	D(mehci->dev, "%s: before msm_hsic_resume_thread\n", __func__);
	...
	D(mehci->dev, "%s: after msm_hsic_resume_thread\n", __func__);
...
static irqreturn_t msm_hsic_wakeup_irq(int irq, void *data)
{
	...
	D(mehci->dev, "%s: hsic remote wakeup interrupt cnt: %u\n",
...
static int __devinit ehci_hsic_msm_probe(struct platform_device *pdev)
{
	...
	D(&pdev->dev, "ehci_msm-hsic probe\n");
	...
	if (ret)
		D(&pdev->dev, "mode debugfs file is"
			"not available\n");
...
static int msm_hsic_pm_suspend(struct device *dev)
{
	...
	D(dev, "ehci-msm-hsic PM suspend\n");
...
static int msm_hsic_pm_suspend_noirq(struct device *dev)
{
	...
	if (atomic_read(&mehci->async_int)) {
		D(dev, "suspend_noirq: Aborting due to pending interrupt\n");
		return -EBUSY;
	}
...
static int msm_hsic_pm_resume(struct device *dev)
{
	...
	if (!atomic_read(&mehci->pm_usage_cnt) &&
		...
		D(dev, "ehci-msm-hsic PM resume - skip resume\n");
		return 0;
	}
