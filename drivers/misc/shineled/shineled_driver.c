...
static ssize_t
sled_grppwm_store(struct device *dev, struct device_attribute *attr,
...
static ssize_t
sled_grpfreq_store(struct device *dev, struct device_attribute *attr,
		   const char *buf, size_t count)
{
	unsigned long value = simple_strtoul(buf, NULL, 10);
	D("%s: buf = %s: count = %d\n", __func__, buf, count);
...
static ssize_t
sled_blink_store(struct device *dev, struct device_attribute *attr,
		 const char *buf, size_t count)
{
	unsigned long value = simple_strtoul(buf, NULL, 10);
	u8 t123, t4;
...
static void
lcds_set_brightness(struct led_classdev *led_cdev, enum led_brightness value)
{
	D("%s:\n", __func__);
...
static int
SN3193_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	int ret = 0;
	int i;
